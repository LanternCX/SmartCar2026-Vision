...
- `assistant/main_v2.py` 的找物体主线与 `master/main_v2.py` 对齐, 每帧先缓存 YOLO 候选, 再按当前任务消费候选并输出速度或事件。
...
- `assistant/main_v2.py` 的 YOLO 入口使用独立 `yolo_detect` 步骤, 调用顺序与主车 `main_v2` 保持一致。
