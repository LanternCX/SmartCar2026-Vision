...
- 主车搜索目标点可通过 `MASTER_SEARCH_TARGET_X_PX` 与 `MASTER_SEARCH_TARGET_Y_PX` 调整，默认按 QVGA `320x240` 使用 `x=160` 与 `y=240`。
...
- 使用 `a,<seq>` 确认本地同步包。
- 找到目标物体后输出可靠事件 `r,<seq>,6,<value>`。


## 辅车找物体规则

- 找物体模式使用红色目标阈值，与主车目标搜索保持一致。
- 找物体目标点可通过 `OBJECT_APPROACH_TARGET_X_PX` 与 `OBJECT_APPROACH_TARGET_Y_PX` 调整，默认按 QVGA `320x240` 使用 `x=160` 与 `y=240`。
- 横向控制目标为目标中心对齐当前目标点。
- 纵向控制目标为目标底边对齐当前目标点。
- 无有效目标时使用配置的搜索速度。
- 目标满足面积下限并连续进入横向、纵向容差窗口后回报 `TARGET_FOUND`。
- 未确认的 `TARGET_FOUND` 事件按低频节奏重复发送。
