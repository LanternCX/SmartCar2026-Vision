...
1. 图像采集与目标识别
2. 在 ART 端完成跟随阶段判断
3. 直接生成并持续发送底盘已支持的速度式命令

也就是说，当前主链路口径就是由 OpenArt 直接发送 `vx=<vx>,vy=<vy>`。

视觉端不额外扩第二套视觉协议，而是直接对齐底盘当前已经支持的速度命令格式。
...
- 纵向控制输入来自目标面积相对目标面积的误差

### 2. 速度式命令发送

视觉端在主循环中逐帧发送速度式命令，当前主线协议格式为：
...
- 无目标或保持阶段都发送 `vx=0,vy=0`
- 当前主线直接对齐底盘已经支持的速度命令入口

### 3. 责任边界

- OpenArt: 负责看见目标、选中目标、判断当前跟随阶段并生成 `vx/vy` 速度量
- RT1021: 负责直接消费并执行 ART 已经生成好的速度命令
